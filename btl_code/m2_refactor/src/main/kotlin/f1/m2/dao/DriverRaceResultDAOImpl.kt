...
//        public final var team: Team
    ...
    override fun findAllByRace(rid: Int): ArrayList<DriverRaceResult> {
        val results = ArrayList<DriverRaceResult>()
        //
        val sql = """
            SELECT 
                d.id as id, d.laps as laps, d.time as time, d.point as point, d.position as position, d.startingPos as startingPos,
                r.id as raceId, r.name as raceName, r.numberOfLaps as numberOfLaps,
                dr.id as driverId, dr.number as driverNumber, dr.name as driverName, 
                t.id as teamId, t.name as teamName
            FROM tblDriverRaceResult as d
            JOIN tblRace as r
            ...
            pstm.executeQuery().use {
                while (it.next()) {
                    val result = DriverRaceResult(
                        ...
                        point = it.getInt("point"),
                        position = it.getInt("position"),
                        startingPos = it.getInt("startingPos"),
                        race = Race(
                            id = it.getInt("raceId"),
                            name = it.getString("raceName"),
    ...
    override fun updateN(results: ArrayList<DriverRaceResult>): Boolean {
        val sql = """
            UPDATE tblDriverRaceResult
            SET
                startingPos=?,
                position=?,
                laps=?,
        ...
        try {
            ...
            conn.prepareStatement(sql).use { pstm ->
                for (r in results) {
                    pstm.setInt(1, r.startingPos)
                    pstm.setInt(2, r.position)
                    pstm.setInt(3, r.laps)
                    pstm.setLong(4, r.time)
                    pstm.setInt(5, r.point)
                    pstm.setInt(6, r.id)
